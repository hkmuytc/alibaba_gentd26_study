...
def main():
    print(f"=== LOCALITY ANALYSIS — POWER DOMAIN (70/15/15, w={WINDOW}) ===\n")
...
def fig_power_error_growth(results, feat_df, n_gpus):
    ...
    ax1.set_title("Multi-Step Window-Averaged Power Prediction Across Forecast Horizons\n"
                  "(70/15/15 Split, Transformer, w=60)")
...
def fig_per_sample_advantage(results, power_kw):
    ...
    ax.set_title(
        f"Per-Sample Power Prediction Advantage at h={H_FOCUS} min (70/15/15)\n"
...
def _write_locality_table(results, y_all, gpu_vol, regimes):
    ...
    lines.append("")
    lines.append(f"**Configuration**: 70/15/15 split, w=60, Transformer, h={H_FOCUS}")
