...
def _write_multistep_table(all_results):
    """Generate markdown table for 70/15/15 split."""
    split = all_results.get("70/15/15", {})
    if not split:
        return
    ...
    lines.append("")
    lines.append("**Configuration**: 70/15/15 split, w=60, Transformer, residual prediction")
