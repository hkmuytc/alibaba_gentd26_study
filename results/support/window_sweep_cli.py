...
def print_data_and_cache_policy() -> None:
    ...
    print("  Lookback cache: results/window_sweep/lookback_results.json -> skips completed fold/window evaluations")
    print("  Horizon cache: results/window_sweep/horizon_results.json -> skips completed fold/model horizon evaluations")
    print("  Model artifacts: saved to models/saved as window-specific timestamped runs")
    print("  Each window size is trained as its own model configuration")
    print("  Rolling-origin evaluation repeats train/validation/test splits from several forecast start points")
    print("  Matching saved model artifacts may be reused for the same architecture/window/feature schema")
    print("  Use --rebuild-study-results to ignore the JSON study caches and retrain/recompute the sweeps")
    print("  Additional analysis reuses the same fixed-window model artifacts when available")
    print()
